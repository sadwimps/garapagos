...
def fncChkAdv():
    tmp = 0
    chkAdvPlus = sum(i > 0  for i in g_lstPlaceStat)
    chkAdvMinus = sum(j < 0  for j in g_lstPlaceStat)
    if g_ivTeban.get() < 0:
        tmp = chkAdvPlus
        chkAdvPlus = chkAdvMinus
        chkAdvMinus = tmp

    return chkAdvPlus - chkAdvMinus
...
#           周囲の各数の個数(インデックスが...0:0の数、1:1の数、2:2以上の数、3:-1の数、4:-2以上の数、5:影響を受けない数、6:インデックス、7:重み)
#           ※自分の番手の影響を受けない様に後手の場合は個数の値が逆となっている。
# ----------------------------------------------------------
def fncChkCondOnAroundLiveList(list_CondFor,list_ChkFor, list_Weight):

    l_chkNum_AroundOnLiveList = [[0 for t in range(8)] for u in range(len(list_ChkFor))]
    k = 0 #配列の動きを制御する変数
    for i in list_ChkFor:
        ...
        for t in list_Weight:
            if i in t[0]:
                l_chkNum_AroundOnLiveList[k][7] = t[1][0]
...
def fncDicision2dmListIndex(l_CondFor):
    r_CondFor = l_CondFor * g_ivTeban.get()
...
def fncWin(a_intTF):
    ...
    tkmsg.showinfo(strMassage[0],strMassage[1])
