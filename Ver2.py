...
import tkinter.messagebox as tkmsg   # メッセージボックスモジュール
...
# 考え始めるよ------------------------------------------------------
    ...
    if loop > 0:
        for i in range(4,loop):
            TakeStoneList = fncCheckEnd(i,l_liveStone,1,takePairNum)
    ...
    #中盤は2のペアを偶数個で返す様にとる！
    for chkPair2 in fncAllForOne(g_lstLiveStone,'4321'):
        l_chkliveStone = list(g_lstLiveStone)
        for chkRemove in chkPair2:
            l_chkliveStone.remove(chkRemove)
        if len(fncAllForOne(l_chkliveStone,'2')) % 2 == 0:
           return fncGetStoneStr(chkPair2)
    ...
    if TakeStoneList[0] < 1:
        if zanNum4 > 0:
            ...
            TakeStoneList = list(g_lstLiveStone[go1])
...
def fncCheckEndOne(chkEndList,selectPairNum):
    ...
    for chk in chkOneList:
        ...
        if len(fncAllForOne(l_chkEndList,'4')) == 0 and len(fncAllForOne(l_chkEndList,'3')) == 0 and len(fncAllForOne(l_chkEndList,'2')) == 0 and len(l_chkEndList)%2 == 1:
...
#--------------------------------------------------------------------
    ...
    for chk in chkPairList:
        ...
        if len(l_chkEndList) > 0:
            ...
            loseChk = fncCheckEnd(loopNum,l_chkEndList,slectPairfnc,'4321')[0]
            if loseChk > 0:
                chkListWorkMid.append(-100)
...
def fncWin(a_intTF):
    ...
    if g_ivPlay.get() == G_GOTE:
        if intWinnwr == G_TURE:
            ...
            intWinnwr = G_TURE
    if intWinnwr == G_FALSE:                 # 先手が勝者なら
        strWinner = u'敗北。。。おぬしやるな。。。'       # 先手を勝者としたログ文字を設定
        if asc == 0:
            tkmsg.showinfo('負けた…','き、きいてないゼ…　by 平地\n　　　　∧＿∧\n・; .、(・ω(:;(⊂＝⊂≡ \n　　　　(っΣ⊂≡⊂＝ \n　　　　/ 　　) ズバババ\n　　　　( /￣∪')
        else:
            tkmsg.showinfo('負けた…',' 　　許せん！　by 平地\n　　　∧＿＿∧ \n　|| （＃｀Д´ ） || \n　|| ( |　 　| | ||| \n　从| |　 　| |从从 \n￣) ⊂ノ￣￣!_つ￣て \n　フヘ(　　　　ヽヘ(＼')
    else:                                   # 後手が勝者なら
        if asc == 0:
            tkmsg.showinfo('勝った！！','∧,,∧ \n( >ω< )つ お前は \n（m9 ＼ 　帰ってよし！　by 平地\n　 ＼　 ＼ \n　 　 ) ) ＼ \n　 ／／ ＼ ＼ \n　 (＿） 　 (＿)')
        else:
...
# ----------------------------------------------------------
def fncConectServer():
    fncPrintLog(u'AAAAA')             # エラーメッセージを記録
    ...
    if len(strIpadd) == 0:                             # IPアドレスが空白ならエラー
        ...
        return G_FALSE                                 # エラーを返す
    fncPrintLog(u'IP > ' + strIpadd)             # エラーメッセージを記録
    strPort = g_strPort.get()                          # ポート番号を取得
    fncPrintLog(u'BBBBB')             # エラーメッセージを記録
    if len(strPort) == 0:                              # ポート番号が空白ならエラー
        fncPrintLog(u'ポート番号が指定されていません') # エラーメッセージを記録
        return G_FALSE                                 # エラーを返す
    try:                                               # 例外を監視
        fncPrintLog(u'CCCC')             # エラーメッセージを記録
        fncPrintLog(u'IP > ' + strIpadd)             # エラーメッセージを記録
        fncPrintLog(u'Port > ' + strPort)             # エラーメッセージを記録
        g_skServer.connect((strIpadd, int(strPort)))     # サーバ接続XXXXXXXXXXXXXXXXXXXXXXXXXXXX
        fncPrintLog(u'connect')             # エラーメッセージを記録
        g_skServer.send(strTeam.encode('utf-8'))       # チーム名を送信
        bytRecv = g_skServer.recv(4096)                # 先手後手を受信
        strRecv = bytRecv.decode('utf-8')              # 先手後手を受信
        fncPrintLog(u'strRecv >' + strRecv)             # エラーメッセージを記録
        if int(strRecv) == G_SENTE:                    # 先手なら
            g_ivPlay.set(G_SENTE)                      # 手番表示を先手で初期化
            fncPrintLog(u'先手')           # サーバ接続を記録
        else:                                          # 後手なら
            g_ivPlay.set(G_GOTE)                       # 手番表示を先手で初期化
            g_skServer.send('OK'.encode('utf-8'))      # 後手なら受信待ちの状態にするためOKを送信
            fncPrintLog(u'後手')           # サーバ接続を記録
    except:                                            # 例外が発生したら
        fncPrintLog(u'XXXX')             # エラーメッセージを記録
...
def fncPushSelect() :
    ...
    if intMode == G_MANCOM:                         # 人対通信ならば
        ...
        g_strInput.set(strStones)                   # 思考ロジックの初手を指し手入力へ設定XXXXXXXXXXXXXXXXXXXXXXX
    g_frmStart.lower(g_frmMain)                     # 初期Frameを非表示
    fncPrintLog('----- Selected Mode --> '+strMode) # 対戦モード選択を記録
    strTeam = g_strTeam.get()                       # XXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
def fncPushGet():
    ...
    if fncTakeStones(g_strInput.get()) == G_FALSE:  # 指定された石を取る
        if len(g_lstLiveStone) == 0:                # XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            return                                  # エラーが発生したら返すXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    intMode = g_ivMode.get()                        # 対戦モードを取得
    if intMode == G_MANCOM:                         # 人対通信ならば
        ...
        fncTakeStones(strStones)                    # 受信結果を反映
        strStones = fncThinking()                   # 思考ロジックを呼び出し
        g_strInput.set(strStones)                   # 思考ロジックを指し手入力へ設定XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    else:                                           # 人対PCならば
        g_strInput.set('')                          # 石の指定をクリア
        if len(g_lstLiveStone) > 1:
            strStones = fncThinking()               # 思考ロジックを呼び出し
